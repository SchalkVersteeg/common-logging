...
#if NETCF
			RegisterTypeParser<bool>(Convert.ToBoolean);
			RegisterTypeParser<short>(Convert.ToInt16);
			RegisterTypeParser<int>(Convert.ToInt32);
			RegisterTypeParser<long>(Convert.ToInt64);
			RegisterTypeParser<float>(Convert.ToSingle);
			RegisterTypeParser<double>(Convert.ToDouble);
			RegisterTypeParser<decimal>(Convert.ToDecimal);
#else
...
#if PROFILE259
					T result;
					if (Enum.TryParse<T>(stringValue, true, out result))
						return result;
					else
						throw new ArgumentException(string.Format("WARN: failed converting value '{0}' to type '{1}'", typeof(T).FullName));
#else
