...
namespace Common.Logging
{
	...
	public class LogManager : ILogManager
	{
		...
		/// on <see cref="ConfigurationReader"/>.
		/// </remarks>
		string ILogManager.COMMON_LOGGING_SECTION
		{
			get { return COMMON_LOGGING_SECTION; }
		}
		...
		void ILogManager.Reset(IConfigurationReader reader)
		{
			Reset(reader);
		}
		...
		{
			get { return ConfigurationReader; }
		}
		...
		[Obsolete(
			"Null-Reference Exception when dealing with Dynamic Types, Prefer instead one of the LogManager.GetLogger(...) variants."
			)]
		...
		private static Func<MethodBase> CreateGetClassNameFunction()
		{
			...
			if (stackFrameType == null)
				throw new PlatformNotSupportedException(
					"CreateGetClassNameFunction is only supported on platforms where System.Diagnostics.StackFrame exist");
#if PROFILE259
			throw new PlatformNotSupportedException(
		...
		private static ILoggerFactoryAdapter BuildLoggerFactoryAdapter()
		{
			...
			if (sectionResult == null)
			{
				string message = (ConfigurationReader.GetType() == typeof(DefaultConfigurationReader))
					...
					: string.Format("Custom ConfigurationReader '{0}' returned <null> - suppressing logging output",
			...
			{
#if PORTABLE || NETCF
				Debug.WriteLine(string.Format("Using ILoggerFactoryAdapter returned from custom ConfigurationReader '{0}'",
					ConfigurationReader.GetType().FullName));
#else
...
#if NETCF
									adapter = (ILoggerFactoryAdapter)CreateInstance(setting.FactoryAdapterType, args);
#else
